...
public class AppointmentController {
	...
	public APIresponse setAppointment(@RequestBody AppointmentDummy appointmentDummy, HttpServletRequest request) {
		...
		for (AppointmentDate date : dates) {
				String[] formattedDate= date.getDate().split("T");
				date.setDate(formattedDate[0]+" "+formattedDate[1]);
			...
			for (AppointmentTime time : times) {
				...
				//Added later to format start and end time
				if(time.getStartTime().length() != 7) {
					String newDateString = null;
					String[] strings = new String[2];
					strings=time.getStartTime().split(":");
					if(strings[0].length() != 2) {
						newDateString = "0"+strings[0]+":"+strings[1];
					}
					time.setStartTime(newDateString);

				}
				if(time.getEndTime().length() != 7) {
					String newDateString = null;
					String[] strings = new String[2];
					strings=time.getEndTime().split(":");
					if(strings[0].length() != 2) {
						newDateString = "0"+strings[0]+":"+strings[1];
					}
					time.setEndTime(newDateString);
